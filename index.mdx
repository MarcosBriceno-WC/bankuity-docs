---
title: Welcome to Bankuity
description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.'
mode: "custom"
---

import { HomeBg } from "/snippets/homebg.jsx"

<HomeBg/>

<div className="overflow-hidden bg-[#11132D] px-6 pt-14 lg:px-8">
  
  {/* Hero Section */}
  <div className="mx-auto max-w-2xl py-24 text-center">
    <div className="flex justify-center mb-8">
      <img noZoom src="/logo/dark.svg" alt="Bankuity Docs logo" />
     </div> 
    <p className="mt-6 text-lg leading-8 text-gray-400 max-w-xl mx-auto">
      Dive into our docs and learn how to leverage financial analysis, income verification, and risk assessment through the Bankuity API.
    </p>
  </div>

  {/* Cards Grid */}
  <div className="mx-auto max-w-7xl pb-24">
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      
      {/* Card 1: What is Bankuity? */}
      <a href="/introduction" className="group rounded-2xl border border-white/10 bg-[#111224] overflow-hidden hover:border-cyan-500/50 transition-all">
        <div className="bg-[#111224] relative flex items-center justify-center">
          <img src="/logo/home-1.svg" />
        </div>
        <div className="p-6">
          <h3 className="text-lg font-semibold text-white">What is Bankuity?</h3>
          <p className="mt-2 text-sm text-gray-400 leading-relaxed">
            Discover how our behavioral analytics and transaction enrichment provide deep insights into financial stability.
          </p>
        </div>
      </a>

      {/* Card 2: Get Started */}
      <a href="/quickstart" className="group rounded-2xl border border-white/10 bg-[#111224] overflow-hidden hover:border-cyan-500/50 transition-all">
        <div className="bg-[#111224] relative flex items-center justify-center">
          <img src="/logo/home-2.svg" />
        </div>
        <div className="p-6">
          <h3 className="text-lg font-semibold text-white">Get Started</h3>
          <p className="mt-2 text-sm text-gray-400 leading-relaxed">
            Learn how to authenticate into our API and perform your first analysis request.
          </p>
        </div>
      </a>

      {/* Card 3: API Reference */}
      <a href="/api-reference" className="group rounded-2xl border border-white/10 bg-[#111224] overflow-hidden hover:border-cyan-500/50 transition-all">
        <div className="bg-[#111224] relative flex items-center justify-center">
          <img src="/logo/home-3.svg" />
        </div>
        <div className="p-6">
          <h3 className="text-lg font-semibold text-white">API Reference</h3>
          <p className="mt-2 text-sm text-gray-400 leading-relaxed">
            Explore endpoints for income analysis, bank account verification, and automated lending recommendations.
          </p>
        </div>
      </a>
    </div>
  </div>
</div>